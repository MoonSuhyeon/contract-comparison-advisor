...
그 결과를 확인·수정할 수 있도록 하는 계약 비교 프로세스**를 만들었습니다.
...
**AI가 먼저 계약을 비교하고 사람이 검토하는 방식**으로 바꿨습니다. 기존 계약·신규
계약·고객 상담 내용을 입력하면 AI가 계약 간 변경사항을 찾아 `changes`로 정리하고, 각
변경사항의 원문 근거(`evidence`)와 고객이 중요하게 언급한 내용(`customer_needs`,
`linked_need_id`)을 함께 연결합니다. AI가 확정하기 어려운 정보는 `unknowns`, 계약서나
출처 간 충돌은 `conflicts`로 분리해 사람이 확인할 수 있도록 했습니다.

즉, **사람이 계약서 전체를 처음부터 비교하는 대신 AI가 먼저 차이를 찾아주고, 사람은 AI가
제시한 차이와 근거를 중심으로 확인·수정·확정하는 흐름**입니다. 최종 판단은 사람이 하도록
역할을 나눴습니다.

이 방식이 실제로 검증 가능한지도 확인했습니다. 15개의 테스트 케이스와 Ground Truth를
설계하고, 그중 7개를 structured 파이프라인과 naive 프롬프트로 각각 실행했습니다. 같은
항목 단위로 다시 비교한 결과 주요 변경사항 발견 성능은 두 방식 모두 14/15로 같았습니다.
따라서 **structured 방식이 계약 차이를 더 잘 찾아낸다고 주장하지 않고**, 변경사항·근거·
충돌·미확인 정보를 구조화해 AI의 결과를 사람이 검증할 수 있도록 만든 데 의미를 두었습니다.
이 과정에서 평가기(`src/evaluate.py`) 자체도 검증해 9건의 버그를 찾아 수정했습니다.

또한 Ground Truth가 없는 실제 신규 계약에서도 활용할 수 있도록, LLM이나 Ground Truth에
의존하지 않고 원문만으로 계약 간 충돌을 탐지하는 `conflict_detector.py`와 사람이 결과를
확인·수정하는 검토 화면(`src/review_app.py`)을 구현했습니다.

마지막으로 **AI를 활용한 계약 비교가 실제 작업량을 줄이는지도 자기실험으로 확인했습니다.**
실험자 1인이 7개 케이스를 `AI 없이 직접 작성`하는 조건과 `AI 초안 검토·수정` 조건으로
각각 처리해 총 14세션을 실행했습니다. 2026-09-18 기준 AI 초안이 있을 때 작업시간은 평균
**75.6% 감소**했습니다. 다만 이는 실험자 1인의 자기실험(N=7) 결과이며, AI가 사람보다
계약 차이를 더 잘 찾는다는 의미는 아닙니다. 또한 AI가 놓친 Conflict를 사람이 능동적으로
발견한 사례는 이번 실험에서 확인하지 못해, **업무 속도 개선과 AI 오류 검증 능력은 별개의
과제로 남았습니다.**

자세한 비교 결과와 평가기 버그 9건, GT 없이 수행한 conflict 탐지, 세 번째 baseline 검증,
14세션 업무 효과 실험 전체 기록은 [`04_검증결과.md`](04_검증결과.md),
[`05_업무효과검증_설계.md`](05_업무효과검증_설계.md), [`06_종합정리.md`](06_종합정리.md),
