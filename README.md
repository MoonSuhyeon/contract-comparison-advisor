...
기존 보험계약을 신규 상품으로 바꿀 때, 보장·지급조건·갱신·환급금처럼 눈에 잘 안 띄는 조건
변화를 놓치면 고객이 손해를 볼 수 있습니다. 금융위원회도 GA(법인보험대리점)의 이런 "중요사항
비교 미고지"를 반복되는 불완전판매 문제로 지목한 바 있습니다. 이 프로젝트가 묻는 질문은
"AI가 계약을 잘 비교하는가"가 아니라 "AI의 출력을 어떻게 검증하고, 실패를 사람이 개입
가능한 데이터로 남기는가"입니다.
...
AI는 계약 간 차이를 찾아 근거와 함께 보여주는 역할까지만 맡고, 최종 판단은 사람이 하도록
역할을 나눴습니다. 테스트 케이스 15개(Ground Truth 포함)를 설계해 그중 7개를 구조화
파이프라인과 naive 프롬프트 양쪽으로 실행·비교했고, 그 과정에서 평가기(`src/evaluate.py`)
자체의 버그도 여러 건 찾아 고쳤습니다.

핵심 발견은, naive와 structured가 "변화를 찾아내는 능력" 자체는 비슷했지만 근거 명시·출처
충돌 보존·판단 경계 준수에서는 크게 갈렸다는 것입니다. 더 중요한 건 structured의 실패는
평가기가 자동으로 감지해 사람에게 넘길 수 있었던 반면, naive의 실패는 사람이 원문을 전부
다시 봐야만 드러났다는 점입니다.

자세한 비교표·평가기 버그 9건·추가 검증(GT 없는 conflict 탐지, 세 번째 baseline)은
[`06_종합정리.md`](06_종합정리.md), [`04_검증결과.md`](04_검증결과.md),
