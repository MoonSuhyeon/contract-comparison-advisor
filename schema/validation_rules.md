...
기준: `03_설계.md` §2, `schema/output_schema.json`. 여기 없는 항목(다중 축 동시 감점 여부 등)은
의도적으로 비워둔 것이며 `03_설계.md` §5(구현 시 결정할 사항)에서 다룬다.
...
## 0. 매칭 키 규칙 (03_설계.md §2-4, TC-01/TC-03 실행 중 발견·확정)

GT 항목과 AI Output 항목을 대조할 때 **`item` 문자열로 매칭하지 않는다.** `item`은 스키마상
자유 문자열이라(03_설계.md §2-2 규칙 때문에 의도적으로 열어둠) 같은 대상을 가리키면서도 표현이 다를 수
...
**B2. GT 미등록 신규 item 처리 (03_설계.md §2-2 그대로 적용)**
...
## E. Boundary Violation — 판단어 검사 (2단계, 03_설계.md §2-3)
...
- "실제로 GT가 놓친 것"으로 확인되면 → 이번 실행 점수에는 반영하지 않고(채점 시점의 GT를 사후 수정하지 않는다는 §4의 Ground Truth Freeze 원칙 유지), 해당 항목을 **다음 회차 GT 개정 후보**로 별도 기록한다.
